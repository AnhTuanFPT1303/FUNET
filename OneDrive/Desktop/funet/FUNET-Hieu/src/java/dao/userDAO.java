...
public class userDAO {
    ...
    public boolean login(String email, String password) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                byte[] storedHashBytes = Base64.getDecoder().decode(HashPassword);

                // Tách salt ra khỏi chuỗi
                byte[] saltBytes = new byte[16]; 
                System.arraycopy(storedHashBytes, 0, saltBytes, 0, 16);

                // Hash lại mật khẩu người dùng với salt đã tách
                String hashedInputPassword = SHA512WithSalt.hashPassWordWithSHA512(password, saltBytes);

                // So sánh
                if (HashPassword.equals(Base64.getEncoder().encodeToString(hashedInputPassword.getBytes()))) {
                    result = true;
                }
            }
        } catch (Exception e) {
    ...
    public String register(User user) {
        try {
            // Lưu trữ mã hash thay cho mật khẩu
            byte[] salt = SHA512WithSalt.createSalt();
            String hashedPassword = SHA512WithSalt.hashPassWordWithSHA512(user.getPassword(), salt);
            
            Connection conn = sqlConnect.getInstance().getConnection();
            CallableStatement st = conn.prepareCall("INSERT INTO userAccount (first_name, last_name, password, email, profile_pic) VALUES (?,?,?,?,?)");
            st.setString(1, user.getFirst_name());
            st.setString(2, user.getLast_name());
            st.setString(3, Base64.getEncoder().encodeToString(hashedPassword.getBytes()));
