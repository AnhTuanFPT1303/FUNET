...
public class ConversationDAO {
    ...
    private void createNewConversation(Connection conn, Conversation conversation, List<User> users) throws Exception {
        String createConversationSQL = "INSERT INTO conversation(conversation_name, conversation_avater) VALUES(?, CONCAT('group-', CAST(IDENT_CURRENT('conversation') AS CHAR(50))))";
    ...
    private void addUserToConversation(Connection conn, int conversationId, User user) throws Exception {
        String addUserSQL = "INSERT INTO conversations_users(conversation_id, user_id, is_admin) VALUES(?, ?, ?)";
    ...
    private void updateConversation(Connection conn, Conversation conversation) throws Exception {
        String updateSQL = "UPDATE conversation SET conversation_name=?, conversation_avater=? WHERE conversation_id=?";
    ...
    public List<Conversation> findAllConversationsByUserId(int userId) throws Exception {
        Connection conn = sqlConnect.getInstance().getConnection();
        String query = "SELECT c.conversation_id, c.conversation_name, c.conversation_avater "
                + "FROM conversation c JOIN conversations_users cu ON c.conversation_id = cu.conversation_id "
                + "WHERE cu.user_id = ?";
        ...
        while (rs.next()) {
            Conversation conversation = new Conversation(rs.getInt("conversation_id"), rs.getString("conversation_name"), rs.getString("conversation_avater"));
    ...
    public Conversation findConversationById(int id) throws Exception {
        Connection conn = sqlConnect.getInstance().getConnection();
        String query = "SELECT c.conversation_id, c.conversation_name, c.conversation_avater FROM conversation c WHERE c.conversation_id = ?";
        PreparedStatement st = conn.prepareStatement(query);
        st.setInt(1, id);
        ResultSet rs = st.executeQuery();
        Conversation conversation = null;
        if (rs.next()) {
            conversation = new Conversation(rs.getInt("conversation_id"), rs.getString("conversation_name"), rs.getString("conversation_avater"));
        }
        conn.close();
        return conversation;
    }
    ...
    public void deleteConversationById(int id) throws Exception {
        Connection conn = sqlConnect.getInstance().getConnection();
        try {
            String deleteSQL = "DELETE FROM conversations_users WHERE conversation_id = ?;"
    ...
    public void deleteUserFromConversation(int conversationId, int userId) throws Exception {
        Connection conn = sqlConnect.getInstance().getConnection();
        String deleteSQL = "DELETE FROM conversations_users WHERE conversation_id = ? AND user_id = ?";
        PreparedStatement st = conn.prepareStatement(deleteSQL);
        st.setInt(1, conversationId);
        st.setInt(2, userId);
        st.executeUpdate();
    ...
    public List<Conversation> findConversationsOfUserByKeyword(int userId, String keyword) throws Exception {
        Connection conn = sqlConnect.getInstance().getConnection();
        String query = "SELECT c.conversation_id, c.conversation_name, c.conversation_avater "
                + "FROM conversation c JOIN conversations_users cu ON cu.conversation_id = c.conversation_id "
